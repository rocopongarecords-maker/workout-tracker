export const exerciseInfo = {
  // ========== BLOCK 1 — LEGS #1 ==========
  "Back Squat": {
    imageUrl: "https://media.musclewiki.com/media/uploads/videos/branded/male-Barbell-barbell-squat-front.mp4#t=0.001",
    muscles: ["Quadriceps", "Glutes", "Core"],
    tips: [
      "Brace your core hard before descending and maintain tightness throughout",
      "Drive your knees out over your toes and sit between your hips",
      "Keep your chest up and maintain a neutral spine"
    ],
    mistakes: [
      "Letting the knees cave inward during the ascent",
      "Rising hips-first and turning the squat into a good-morning"
    ]
  },
  "Deadlift": {
    imageUrl: "https://media.musclewiki.com/media/uploads/videos/branded/male-Barbell-deadlift-front.mp4#t=0.001",
    muscles: ["Hamstrings", "Glutes", "Lower Back", "Traps"],
    tips: [
      "Set up with the bar over mid-foot, shins nearly touching the bar",
      "Pull the slack out of the bar before initiating the lift",
      "Drive through the floor and lock out with your glutes at the top"
    ],
    mistakes: [
      "Rounding the lower back off the floor",
      "Letting the bar drift away from the body during the pull"
    ]
  },
  "Barbell Hip Thrust": {
    imageUrl: "https://media.musclewiki.com/media/uploads/videos/branded/male-Barbell-barbell-hip-thrust-front.mp4",
    muscles: ["Glutes", "Hamstrings"],
    tips: [
      "Position your upper back on the bench at the bottom of your shoulder blades",
      "Drive through your heels and squeeze your glutes hard at lockout",
      "Keep your chin slightly tucked to maintain a neutral spine"
    ],
    mistakes: [
      "Hyperextending the lower back at the top instead of using glute squeeze",
      "Placing the bar too high on the hips causing discomfort"
    ]
  },
  "Dumbbell Walking Lunge": {
    muscles: ["Quadriceps", "Glutes", "Hamstrings"],
    tips: [
      "Take a long enough stride so your front knee stays over the ankle",
      "Keep your torso upright and core braced throughout the movement",
      "Control the descent and lightly touch the back knee to the floor"
    ],
    mistakes: [
      "Taking too short a stride which shifts all the load to the knee",
      "Leaning the torso too far forward and losing balance"
    ]
  },
  "Leg Extension": {
    imageUrl: "https://media.musclewiki.com/media/uploads/videos/branded/male-Machine-leg-extension-front.mp4",
    muscles: ["Quadriceps"],
    tips: [
      "Adjust the pad so it sits just above your ankles",
      "Squeeze hard at full extension and hold briefly for peak contraction",
      "Use a controlled 2-second eccentric on the way down"
    ],
    mistakes: [
      "Using momentum to swing the weight up",
      "Setting the seat too far back so your knees are ahead of the pivot point"
    ]
  },
  "Seated Leg Curl": {
    imageUrl: "https://media.musclewiki.com/media/uploads/videos/branded/male-Machine-seated-leg-curl-front.mp4",
    muscles: ["Hamstrings"],
    tips: [
      "Adjust the machine so the pad sits just above your heels",
      "Curl all the way down and squeeze your hamstrings at full contraction",
      "Control the eccentric — don't let the weight stack slam"
    ],
    mistakes: [
      "Lifting your hips off the seat to generate momentum",
      "Using too short a range of motion"
    ]
  },
  "Standing Calf Raise": {
    muscles: ["Calves"],
    tips: [
      "Get a full stretch at the bottom by letting your heels drop below the platform",
      "Pause and squeeze at the top for 1-2 seconds",
      "Keep your knees straight but not hyperextended"
    ],
    mistakes: [
      "Bouncing at the bottom instead of using a full controlled range of motion",
      "Bending the knees and turning it into a partial squat"
    ]
  },

  // ========== BLOCK 1 — LEGS #2 ==========
  "Front Squat": {
    imageUrl: "https://media.musclewiki.com/media/uploads/videos/branded/male-Barbell-front-squat-front.mp4",
    muscles: ["Quadriceps", "Glutes", "Core", "Upper Back"],
    tips: [
      "Keep your elbows high and parallel to the floor throughout",
      "Maintain an upright torso — the bar should stay over mid-foot",
      "Descend as deep as mobility allows while keeping a neutral spine"
    ],
    mistakes: [
      "Letting the elbows drop which causes the bar to roll forward",
      "Leaning too far forward and losing the front-rack position"
    ]
  },
  "Cable Pull Through": {
    muscles: ["Glutes", "Hamstrings"],
    tips: [
      "Hinge at the hips and push them back, keeping a soft knee bend",
      "Squeeze your glutes hard to drive the hips forward at lockout",
      "Stand far enough from the cable to maintain tension at the top"
    ],
    mistakes: [
      "Squatting the movement instead of hinging at the hips",
      "Pulling with the arms instead of driving with the glutes"
    ]
  },
  "Single-Leg Leg Press": {
    muscles: ["Quadriceps", "Glutes"],
    tips: [
      "Place your foot in the center of the platform at a comfortable height",
      "Lower the sled until your knee reaches at least 90 degrees",
      "Press through the full foot — don't let your heel lift off"
    ],
    mistakes: [
      "Letting the lower back round off the pad at the bottom",
      "Locking out the knee aggressively at the top"
    ]
  },
  "Single-Leg Leg Extension": {
    muscles: ["Quadriceps"],
    tips: [
      "Use a lighter load than bilateral and focus on peak contraction",
      "Squeeze at the top for a full second before lowering",
      "Keep your hips and back flat against the seat"
    ],
    mistakes: [
      "Using too much weight and compensating with a hip shift",
      "Rushing through reps without controlling the eccentric"
    ]
  },
  "Swiss Ball Single-Leg Leg Curl": {
    muscles: ["Hamstrings", "Glutes", "Core"],
    tips: [
      "Dig your heel into the ball and lift your hips into a bridge before curling",
      "Keep your hips high throughout — don't let them sag between reps",
      "Control the ball back out slowly to maintain tension"
    ],
    mistakes: [
      "Dropping the hips between reps and losing glute engagement",
      "Using both legs to cheat on the curl portion"
    ]
  },

  // ========== BLOCK 1 — PUSH #1 ==========
  "Barbell Bench Press": {
    imageUrl: "https://media.musclewiki.com/media/uploads/videos/branded/male-Barbell-bench-press-front.mp4",
    muscles: ["Chest", "Triceps", "Front Delts"],
    tips: [
      "Retract and depress your shoulder blades before unracking",
      "Touch the bar to your lower chest with elbows at roughly 45-75 degrees",
      "Drive your feet into the floor and press in a slight arc back toward the rack"
    ],
    mistakes: [
      "Flaring the elbows to 90 degrees which overloads the shoulders",
      "Bouncing the bar off the chest instead of using a controlled touch"
    ]
  },
  "Dumbbell Seated Shoulder Press": {
    muscles: ["Front Delts", "Lateral Delts", "Triceps"],
    tips: [
      "Start with the dumbbells at ear height, palms facing forward",
      "Press up and slightly inward so the dumbbells nearly touch at the top",
      "Keep your back against the pad and avoid excessive arching"
    ],
    mistakes: [
      "Arching the lower back excessively turning it into an incline press",
      "Pressing the dumbbells too far in front of the body"
    ]
  },
  "Weighted Dip": {
    imageUrl: "https://media.musclewiki.com/media/uploads/videos/branded/male-Bodyweight-dips-front.mp4",
    muscles: ["Chest", "Triceps", "Front Delts"],
    tips: [
      "Lean your torso slightly forward to emphasize the chest",
      "Lower until your upper arms are at least parallel to the floor",
      "Keep your elbows from flaring out excessively wide"
    ],
    mistakes: [
      "Cutting the range of motion short and only doing half reps",
      "Swinging the body and using momentum to get out of the bottom"
    ]
  },
  "Low-to-High Cable Flye": {
    muscles: ["Upper Chest"],
    tips: [
      "Set the pulleys to the lowest position and step forward for tension",
      "Sweep your arms upward in an arc — think about hugging a barrel overhead",
      "Squeeze your upper chest hard at the top of each rep"
    ],
    mistakes: [
      "Bending the elbows too much and turning it into a press movement",
      "Using too much weight and losing the isolation on the chest"
    ]
  },
  "Dumbbell Isolateral Skull Crusher": {
    muscles: ["Triceps"],
    tips: [
      "Work one arm at a time to maximize the mind-muscle connection",
      "Lower the dumbbell to the side of your forehead with a controlled eccentric",
      "Keep your upper arm vertical — only the forearm should move"
    ],
    mistakes: [
      "Flaring the elbow out instead of keeping the upper arm stationary",
      "Using momentum to swing the dumbbell back up"
    ]
  },
  "Dumbbell Lateral Raise": {
    imageUrl: "https://media.musclewiki.com/media/uploads/videos/branded/male-Dumbbell-dumbbell-lateral-raise-front.mp4",
    muscles: ["Lateral Delts"],
    tips: [
      "Lead with your elbows, not your hands — think about pouring water from a pitcher",
      "Raise to just about shoulder height, no higher",
      "Use a slight forward lean from the hips to better target the side delt"
    ],
    mistakes: [
      "Shrugging the traps and using too much weight",
      "Swinging the torso to generate momentum"
    ]
  },
  "Ab Wheel Rollout": {
    muscles: ["Core", "Rectus Abdominis", "Serratus Anterior"],
    tips: [
      "Start from your knees and brace your core before rolling out",
      "Extend as far as you can control without your lower back arching",
      "Pull back by contracting your abs — don't just fold at the hips"
    ],
    mistakes: [
      "Letting the lower back sag and hyperextend at the bottom",
      "Only rolling out a few inches and not getting full extension"
    ]
  },

  // ========== BLOCK 1 — PUSH #2 ==========
  "Close-Grip Bench Press": {
    muscles: ["Triceps", "Chest", "Front Delts"],
    tips: [
      "Use a shoulder-width grip — hands don't need to be touching",
      "Keep your elbows tucked close to your body throughout the press",
      "Touch the bar to your lower chest and press straight up"
    ],
    mistakes: [
      "Gripping too narrow which puts excessive strain on the wrists",
      "Flaring the elbows out and turning it into a regular bench press"
    ]
  },
  "Military Press": {
    imageUrl: "https://media.musclewiki.com/media/uploads/videos/branded/male-Barbell-military-press-front.mp4",
    muscles: ["Front Delts", "Lateral Delts", "Triceps", "Core"],
    tips: [
      "Start with the bar resting on your front delts at collarbone height",
      "Brace your core and glutes hard to create a stable base",
      "Press the bar straight up, moving your head back slightly to clear the chin"
    ],
    mistakes: [
      "Excessive lower back lean turning it into a standing incline press",
      "Not locking out fully overhead"
    ]
  },
  "Dumbbell Incline Press": {
    muscles: ["Upper Chest", "Front Delts", "Triceps"],
    tips: [
      "Set the bench to approximately 30-45 degrees",
      "Retract your shoulder blades and maintain an arch in your upper back",
      "Lower the dumbbells to the sides of your upper chest with control"
    ],
    mistakes: [
      "Setting the incline too steep which shifts work to the shoulders",
      "Letting the dumbbells drift too far toward the belly"
    ]
  },
  "Pec Deck": {
    muscles: ["Chest"],
    tips: [
      "Adjust the seat so your arms are at chest height when gripping the pads",
      "Lead with your elbows and squeeze the chest hard at peak contraction",
      "Use a slow eccentric and feel the stretch in the outer chest"
    ],
    mistakes: [
      "Using too much weight and only getting a half range of motion",
      "Rounding the shoulders forward instead of keeping the chest open"
    ]
  },
  "Cable Lateral Raise": {
    muscles: ["Lateral Delts"],
    tips: [
      "Stand sideways to the cable with the handle in the far hand",
      "Keep a slight bend in the elbow and raise to shoulder height",
      "Control the negative — the cable provides constant tension"
    ],
    mistakes: [
      "Leaning too far away from the cable stack and using body momentum",
      "Raising above shoulder height and shifting work to the traps"
    ]
  },
  "Cable Triceps Kickback": {
    muscles: ["Triceps"],
    tips: [
      "Hinge forward and pin your upper arm to your side",
      "Fully extend your arm and squeeze the tricep at the top",
      "Use a low cable or adjust the pulley to hip height for optimal angle"
    ],
    mistakes: [
      "Swinging the upper arm instead of keeping it locked in place",
      "Using too much weight and losing the lockout squeeze"
    ]
  },
  "Bicycle Crunch": {
    muscles: ["Rectus Abdominis", "Obliques"],
    tips: [
      "Rotate your torso — bring the shoulder toward the opposite knee, not just the elbow",
      "Extend the opposite leg fully on each rep",
      "Move slowly and deliberately rather than rushing through reps"
    ],
    mistakes: [
      "Pulling on the neck with your hands",
      "Going too fast and using momentum instead of controlled rotation"
    ]
  },

  // ========== BLOCK 1 — PULL #1 ==========
  "1-Arm Lat Pull-In": {
    muscles: ["Lats"],
    tips: [
      "Use a cable set to about head height and step back for tension",
      "Keep your arm relatively straight and pull your elbow toward your hip",
      "Focus on feeling the lat contract — this is an activation exercise"
    ],
    mistakes: [
      "Bending the elbow too much and turning it into a row",
      "Using too much weight and losing the lat isolation"
    ]
  },
  "Pull-Up": {
    imageUrl: "https://media.musclewiki.com/media/uploads/videos/branded/male-Bodyweight-pull-up-front.mp4",
    muscles: ["Lats", "Biceps", "Rhomboids", "Core"],
    tips: [
      "Start from a dead hang with shoulders engaged — don't just hang passively",
      "Drive your elbows down and back to initiate the pull",
      "Get your chin over the bar and lower under full control"
    ],
    mistakes: [
      "Kipping or swinging the body to get over the bar",
      "Cutting the rep short and not getting full extension at the bottom"
    ]
  },
  "Pendlay Row": {
    muscles: ["Upper Back", "Lats", "Rear Delts", "Biceps"],
    tips: [
      "Reset the bar on the floor between every rep — each rep starts from a dead stop",
      "Keep your torso parallel to the floor throughout the pull",
      "Explosively row the bar to your lower chest, then lower with control"
    ],
    mistakes: [
      "Using excessive body English and jerking the torso upward",
      "Not returning the bar fully to the floor between reps"
    ]
  },
  "Machine High Row": {
    muscles: ["Upper Back", "Lats", "Rear Delts"],
    tips: [
      "Adjust the chest pad and pull toward your upper chest or neck",
      "Squeeze your shoulder blades together at peak contraction",
      "Use a full range of motion — let your arms extend fully on the eccentric"
    ],
    mistakes: [
      "Leaning too far back and turning it into a low row",
      "Pulling with the biceps instead of initiating with the back"
    ]
  },
  "Seated Face Pull": {
    imageUrl: "https://media.musclewiki.com/media/uploads/videos/branded/male-Cable-face-pull-front.mp4",
    muscles: ["Rear Delts", "Rotator Cuff", "Traps"],
    tips: [
      "Use a rope attachment and pull toward your forehead",
      "Externally rotate at the end so your fists end up beside your ears",
      "Keep your elbows high — at or above shoulder height"
    ],
    mistakes: [
      "Using too much weight and turning it into a row",
      "Not externally rotating at the end position"
    ]
  },
  "Reverse-Grip EZ-Bar Curl (A1)": {
    muscles: ["Brachioradialis", "Forearms", "Biceps"],
    tips: [
      "Grip the EZ-bar on the outer angles with palms facing down",
      "Keep your elbows pinned to your sides throughout the curl",
      "Use a controlled tempo — this targets the brachioradialis"
    ],
    mistakes: [
      "Letting the elbows drift forward and using front delt to assist",
      "Gripping too tightly and neglecting full wrist extension at the bottom"
    ]
  },
  "Supinated EZ-Bar Curl (A2)": {
    muscles: ["Biceps"],
    tips: [
      "Immediately after the reverse curls, flip to an underhand grip and continue",
      "Squeeze the biceps hard at the top of each rep",
      "Keep the same controlled tempo as the A1 movement"
    ],
    mistakes: [
      "Swinging the body to curl the weight up",
      "Resting too long between the A1 and A2 — these are a superset"
    ]
  },
  "Dumbbell Preacher Curl": {
    muscles: ["Biceps", "Brachialis"],
    tips: [
      "Press the back of your arm flat against the pad — no gap",
      "Lower the dumbbell slowly and feel the stretch at the bottom",
      "Curl up to about 120 degrees of elbow flexion, not all the way to vertical"
    ],
    mistakes: [
      "Lifting the elbow off the pad to cheat the weight up",
      "Dropping the weight too fast on the eccentric and risking elbow strain"
    ]
  },

  // ========== BLOCK 1 — PULL #2 ==========
  "Neutral-Grip Pulldown": {
    imageUrl: "https://media.musclewiki.com/media/uploads/videos/branded/male-Cable-lat-pulldown-front.mp4",
    muscles: ["Lats", "Biceps", "Rhomboids"],
    tips: [
      "Use a V-bar or neutral-grip attachment and lean back slightly",
      "Pull the handle toward your upper chest, driving elbows down",
      "Control the eccentric and feel the lats stretch at the top"
    ],
    mistakes: [
      "Leaning too far back and turning it into a row",
      "Using too much weight and shortening the range of motion"
    ]
  },
  "Cable Seated Elbows-Out Row (A1)": {
    muscles: ["Rear Delts", "Upper Back", "Traps"],
    tips: [
      "Use a rope attachment and row with your elbows flared out high",
      "Pull toward your face or upper chest to target the rear delts and upper back",
      "Squeeze the shoulder blades together at peak contraction"
    ],
    mistakes: [
      "Dropping the elbows and turning it into a standard row",
      "Using momentum by rocking the torso back and forth"
    ]
  },
  "Cable Seated Row (A2)": {
    muscles: ["Lats", "Rhomboids", "Biceps"],
    tips: [
      "Immediately switch to a standard rowing grip after the A1 set",
      "Pull toward your navel with elbows close to your sides",
      "Maintain a proud chest and avoid rounding forward"
    ],
    mistakes: [
      "Excessive trunk flexion and extension to generate momentum",
      "Resting too long between the A1 and A2 — keep the superset tight"
    ]
  },
  "Kneeling Straight-Arm Cable Pull-Over": {
    muscles: ["Lats", "Teres Major"],
    tips: [
      "Kneel facing a high cable and keep your arms nearly straight throughout",
      "Pull the bar or rope down to your thighs in a sweeping arc",
      "Focus on squeezing your lats — think about pulling your elbows toward your hips"
    ],
    mistakes: [
      "Bending the elbows and turning it into a pulldown",
      "Using too much weight and losing the lat isolation"
    ]
  },
  "Snatch-Grip Barbell Shrug": {
    muscles: ["Traps", "Upper Back"],
    tips: [
      "Use a wide snatch grip — roughly double your shoulder-width",
      "Shrug straight up and hold the top for a full second",
      "Keep a slight forward lean to hit more of the mid traps"
    ],
    mistakes: [
      "Rolling the shoulders forward or backward instead of shrugging straight up",
      "Using too much weight and barely moving through any range of motion"
    ]
  },
  "Cable Reverse Flye": {
    muscles: ["Rear Delts", "Rhomboids"],
    tips: [
      "Set the cables at shoulder height and cross-grip the handles",
      "Pull outward with nearly straight arms, leading with the rear delts",
      "Squeeze at full extension and control the return"
    ],
    mistakes: [
      "Bending the elbows excessively and turning it into a face pull",
      "Using too much weight which causes the traps to take over"
    ]
  },
  "Single-Arm Cable Curl": {
    muscles: ["Biceps"],
    tips: [
      "Stand sideways to a low cable and curl with one arm at a time",
      "Keep your elbow pinned at your side throughout the movement",
      "Squeeze the bicep hard at the top and control the negative"
    ],
    mistakes: [
      "Letting the elbow drift forward and using the shoulder to assist",
      "Swinging the torso to generate momentum on heavier sets"
    ]
  },
  "Hammer Curl": {
    muscles: ["Brachioradialis", "Biceps", "Brachialis"],
    tips: [
      "Keep a neutral grip with your palms facing each other throughout",
      "Curl both dumbbells simultaneously with elbows pinned to your sides",
      "Control the eccentric for a full 2-second descent"
    ],
    mistakes: [
      "Swinging the body and using hip drive to curl the weight",
      "Alternating arms in a way that lets one side rest too long"
    ]
  },

  // ========== BLOCK 2 — LEGS #1 ==========
  "Tempo Back Squat": {
    muscles: ["Quadriceps", "Glutes", "Core"],
    tips: [
      "Use a controlled 3-4 second eccentric on the way down",
      "Pause briefly at the bottom before driving back up",
      "Use lighter weight than your regular squat — the tempo increases time under tension"
    ],
    mistakes: [
      "Speeding up the descent and losing the tempo prescription",
      "Relaxing at the bottom and losing core tightness"
    ]
  },
  "Romanian Deadlift": {
    imageUrl: "https://media.musclewiki.com/media/uploads/videos/branded/male-Barbell-romanian-deadlift-front.mp4",
    muscles: ["Hamstrings", "Glutes", "Lower Back"],
    tips: [
      "Push your hips back while keeping a slight knee bend",
      "Lower the bar along your thighs until you feel a deep hamstring stretch",
      "Drive your hips forward to return to the top — don't pull with the back"
    ],
    mistakes: [
      "Rounding the lower back instead of maintaining a neutral spine",
      "Bending the knees too much and turning it into a squat pattern"
    ]
  },
  "Pause Barbell Hip Thrust": {
    muscles: ["Glutes", "Hamstrings"],
    tips: [
      "Pause for 2 seconds at the top with a hard glute squeeze",
      "Keep your chin tucked and ribs down to prevent lower back hyperextension",
      "Drive through your heels and focus on glute contraction, not hip height"
    ],
    mistakes: [
      "Rushing through the pause and not holding long enough at the top",
      "Hyperextending the lower back instead of posterior pelvic tilting at lockout"
    ]
  },
  "Reverse Lunge": {
    muscles: ["Quadriceps", "Glutes", "Hamstrings"],
    tips: [
      "Step back far enough so your front shin stays vertical at the bottom",
      "Lower under control until the back knee lightly touches the floor",
      "Drive up through the front heel to return to standing"
    ],
    mistakes: [
      "Stepping too short and letting the front knee shoot forward",
      "Leaning the torso excessively forward"
    ]
  },
  "Enhanced-Eccentric Leg Extension": {
    muscles: ["Quadriceps"],
    tips: [
      "Extend with both legs, then lower with only one leg for a slow eccentric",
      "Take 3-4 seconds on the single-leg lowering phase",
      "Use a lighter weight than your normal leg extension"
    ],
    mistakes: [
      "Lowering the eccentric too quickly and missing the enhanced stimulus",
      "Using too much weight and not being able to control the single-leg negative"
    ]
  },
  "Lying Leg Curl": {
    muscles: ["Hamstrings"],
    tips: [
      "Adjust the pad to sit just above your Achilles tendon",
      "Curl all the way up and squeeze your hamstrings at peak contraction",
      "Keep your hips pressed into the pad throughout — don't let them rise"
    ],
    mistakes: [
      "Lifting the hips off the pad to assist the curl",
      "Using a partial range of motion and not fully contracting at the top"
    ]
  },

  // ========== BLOCK 2 — LEGS #2 (new exercise) ==========
  "Round-Back 45° Hyperextension": {
    muscles: ["Glutes", "Hamstrings"],
    tips: [
      "Intentionally round your upper back to shift emphasis from the spinal erectors to the glutes",
      "Squeeze your glutes hard at the top of each rep to drive hip extension",
      "Hold a plate at your chest for added resistance once bodyweight becomes easy"
    ],
    mistakes: [
      "Hyperextending at the top — stop when your body is in line with your legs",
      "Using the lower back to drive the movement instead of the glutes"
    ]
  },

  // ========== BLOCK 2 — PUSH #1 ==========
  "Pause Barbell Bench Press": {
    muscles: ["Chest", "Triceps", "Front Delts"],
    tips: [
      "Pause the bar on your chest for a full 1-2 seconds before pressing",
      "Stay tight during the pause — don't let your shoulder blades lose retraction",
      "Use about 10-15% less weight than your regular bench press"
    ],
    mistakes: [
      "Sinking the bar into the chest and losing tightness during the pause",
      "Bouncing the bar even slightly instead of a dead-stop pause"
    ]
  },
  "Standing Dumbbell Shoulder Press": {
    muscles: ["Front Delts", "Lateral Delts", "Triceps", "Core"],
    tips: [
      "Brace your core and squeeze your glutes for a solid standing base",
      "Press the dumbbells overhead in a slight arc so they meet at the top",
      "Don't lean back excessively — keep your ribcage down"
    ],
    mistakes: [
      "Over-arching the lower back to compensate for heavy weight",
      "Pressing the dumbbells too far in front instead of straight overhead"
    ]
  },
  "Cable Flye": {
    muscles: ["Chest"],
    tips: [
      "Set the cables at mid-chest height for a standard flye angle",
      "Keep a slight bend in your elbows and squeeze the chest at the front",
      "Step forward so there is tension throughout the full range of motion"
    ],
    mistakes: [
      "Bending the elbows too much and pressing instead of flying",
      "Using too much weight and losing the chest squeeze"
    ]
  },
  "Barbell Floor Skull Crusher": {
    muscles: ["Triceps"],
    tips: [
      "Lie on the floor and lower the bar to just above your forehead",
      "The floor limits the range of motion — use this to overload the lockout",
      "Keep your upper arms vertical and only move at the elbow"
    ],
    mistakes: [
      "Flaring the elbows out wide which shifts work to the shoulders",
      "Letting the upper arms drift backward and turning it into a pullover"
    ]
  },

  // ========== BLOCK 2 — PUSH #2 (new exercises) ==========
  "Egyptian Lateral Raise": {
    muscles: ["Lateral Delts"],
    tips: [
      "Hold a pole or upright with one hand and lean away to increase the range of motion",
      "Raise the dumbbell from full stretch at your side up to shoulder height",
      "The lean creates tension at the bottom — control the entire range"
    ],
    mistakes: [
      "Not leaning far enough away and losing the bottom-range stretch",
      "Shrugging the trap instead of isolating the lateral delt"
    ]
  },
  "Rope Overhead Triceps Extension": {
    muscles: ["Triceps"],
    tips: [
      "Face away from a high cable and hinge forward slightly at the hips",
      "Extend your arms overhead until fully locked out, then control the return",
      "Keep your elbows close to your head — don't let them flare"
    ],
    mistakes: [
      "Moving at the shoulder instead of only at the elbow joint",
      "Standing too upright which reduces the stretch on the long head"
    ]
  },
  "Hanging Leg Raise": {
    muscles: ["Core", "Rectus Abdominis", "Hip Flexors"],
    tips: [
      "Hang from a bar with a firm grip and engage your lats to prevent swinging",
      "Curl your pelvis up — don't just lift your legs, posteriorly tilt at the top",
      "Lower your legs slowly and stop any swinging before the next rep"
    ],
    mistakes: [
      "Swinging and using momentum instead of controlled leg raises",
      "Only lifting the legs without curling the pelvis, which limits ab activation"
    ]
  },

  // ========== BLOCK 2 — PULL #1 (new exercises) ==========
  "Supinated Pulldown": {
    muscles: ["Lats", "Biceps", "Lower Traps"],
    tips: [
      "Use an underhand grip slightly narrower than shoulder width",
      "Pull the bar to your upper chest and squeeze your lats at the bottom",
      "Lean back slightly and focus on driving your elbows down and back"
    ],
    mistakes: [
      "Leaning too far back and turning it into a rowing motion",
      "Curling the weight down with the biceps instead of pulling with the lats"
    ]
  },
  "Chest-Supported T-Bar Row": {
    muscles: ["Upper Back", "Lats", "Rhomboids", "Rear Delts"],
    tips: [
      "Lie face down on the pad and let your arms hang straight down to start",
      "Row the handles toward your lower chest and squeeze your shoulder blades",
      "The chest support eliminates momentum — use strict form"
    ],
    mistakes: [
      "Lifting your chest off the pad to cheat the weight up",
      "Using too heavy a weight and shortening the range of motion"
    ]
  },
  "Seal Row": {
    muscles: ["Upper Back", "Lats", "Rhomboids"],
    tips: [
      "Lie face down on an elevated bench so your arms can hang freely",
      "Row the bar to the underside of the bench and squeeze hard at the top",
      "This eliminates all momentum — focus on a strong back contraction"
    ],
    mistakes: [
      "Jerking the weight up instead of using a controlled pull",
      "Not using full range of motion — let the arms hang fully extended at the bottom"
    ]
  },
  "Dumbbell One-Arm Row": {
    muscles: ["Lats", "Rhomboids", "Biceps"],
    tips: [
      "Place one hand and knee on a bench and keep your back flat",
      "Row the dumbbell to your hip, driving the elbow up and back",
      "Lower under control and feel the lat stretch at the bottom"
    ],
    mistakes: [
      "Rotating the torso excessively to heave the weight up",
      "Shrugging the shoulder instead of pulling with the lat"
    ]
  },
  "Dumbbell Supinated Curl": {
    muscles: ["Biceps"],
    tips: [
      "Start with a neutral grip and supinate (rotate palms up) as you curl",
      "Squeeze the biceps hard at the top with full supination",
      "Lower slowly and return to the neutral grip at the bottom"
    ],
    mistakes: [
      "Not fully supinating at the top and missing the peak bicep contraction",
      "Using body swing to get the weight up on heavier sets"
    ]
  },

  // ========== BLOCK 2 — PULL #2 (new exercises) ==========
  "Neutral-Grip Pull-Up": {
    muscles: ["Lats", "Biceps", "Brachialis", "Core"],
    tips: [
      "Use parallel handles with palms facing each other",
      "Pull until your chin clears the bar, driving elbows down to your sides",
      "Control the descent fully — no dropping into a dead hang"
    ],
    mistakes: [
      "Kipping or swinging to get over the bar",
      "Not achieving full arm extension at the bottom of each rep"
    ]
  },
  "Single-Arm Pulldown": {
    muscles: ["Lats", "Biceps"],
    tips: [
      "Use a D-handle on a high cable and pull one arm at a time",
      "Focus on driving your elbow down toward your hip for a full lat contraction",
      "Let your arm fully extend at the top to get a deep lat stretch"
    ],
    mistakes: [
      "Rotating the torso to use body momentum",
      "Pulling with the bicep instead of initiating with the lat"
    ]
  },
  "Low-to-High Reverse Flye": {
    muscles: ["Rear Delts", "Rhomboids"],
    tips: [
      "Set a cable to the lowest position and pull up and out with nearly straight arms",
      "Lead with the back of your hand and squeeze the rear delt at the top",
      "Keep your body stable — don't rock or rotate"
    ],
    mistakes: [
      "Using too much weight and bending the arms excessively",
      "Shrugging the traps instead of isolating the rear delts"
    ]
  },
  "Reverse Pec Deck": {
    muscles: ["Rear Delts", "Rhomboids"],
    tips: [
      "Sit facing the pad and adjust the handles to the widest starting position",
      "Open your arms with a slight elbow bend, leading with the elbows",
      "Squeeze the rear delts at full extension and control the return"
    ],
    mistakes: [
      "Using too much weight and only getting half the range of motion",
      "Driving with the hands instead of leading with the elbows"
    ]
  },
  "Dumbbell Hammer Curl": {
    muscles: ["Brachioradialis", "Biceps", "Brachialis"],
    tips: [
      "Keep a neutral grip with thumbs facing up throughout the curl",
      "Pin your elbows to your sides and curl with strict form",
      "Squeeze at the top and lower for a controlled 2-second eccentric"
    ],
    mistakes: [
      "Swinging the body and using momentum to curl",
      "Letting the elbows drift forward during the curl"
    ]
  },
  "High Cable Curl": {
    muscles: ["Biceps"],
    tips: [
      "Set two cables at head height and stand in the center",
      "Curl your fists toward your ears with upper arms parallel to the floor",
      "Squeeze the biceps hard in the fully contracted position"
    ],
    mistakes: [
      "Dropping the elbows below shoulder height during the curl",
      "Using too much weight and losing the peak contraction"
    ]
  },

  // ========== FULL BODY PROGRAM — Additional Exercises ==========
  "Dumbbell Flat Bench Press": {
    muscles: ["Chest", "Triceps", "Front Delts"],
    tips: [
      "Retract your shoulder blades and maintain a slight arch in your upper back",
      "Lower the dumbbells to the sides of your chest with elbows at about 45 degrees",
      "Press up and slightly inward so the dumbbells nearly touch at the top"
    ],
    mistakes: [
      "Flaring the elbows to 90 degrees which overloads the shoulders",
      "Letting the dumbbells drift too low toward the belly instead of mid-chest"
    ]
  },
  "Lat Pulldown": {
    muscles: ["Lats", "Biceps", "Rhomboids"],
    tips: [
      "Lean back slightly and pull the bar to your upper chest",
      "Drive your elbows down and back — think about putting them in your back pockets",
      "Control the eccentric and feel a full lat stretch at the top"
    ],
    mistakes: [
      "Leaning too far back and turning it into a rowing motion",
      "Pulling the bar behind the neck which stresses the shoulders"
    ]
  },
  "Incline Barbell Bench Press": {
    muscles: ["Upper Chest", "Front Delts", "Triceps"],
    tips: [
      "Set the bench to 30-45 degrees and unrack with a strong shoulder retraction",
      "Lower the bar to your upper chest just below the collarbone",
      "Press in a slight arc back toward the rack for optimal force"
    ],
    mistakes: [
      "Setting the incline too steep which turns it into a shoulder press",
      "Bouncing the bar off the chest instead of using a controlled touch"
    ]
  },

  // ========== HYROX PROGRAM — Station & Conditioning Exercises ==========
  "Ski Erg": {
    muscles: ["Lats", "Triceps", "Core", "Shoulders"],
    tips: [
      "Start with arms overhead and hinge at the hips as you pull down",
      "Drive the handles down past your hips with a powerful lat contraction",
      "Use your legs by bending the knees slightly to generate more power"
    ],
    mistakes: [
      "Only using your arms and not engaging the hips and core",
      "Standing too upright — a strong hip hinge is key to power output"
    ]
  },
  "Sled Push": {
    muscles: ["Quadriceps", "Glutes", "Calves", "Core"],
    tips: [
      "Grip the high handles, lean forward at 45 degrees, and drive through your toes",
      "Keep your back flat and core braced — don't round the spine",
      "Take short, powerful steps and maintain steady forward momentum"
    ],
    mistakes: [
      "Standing too upright which reduces pushing power",
      "Taking too long strides — short choppy steps are more efficient"
    ]
  },
  "Sled Pull": {
    muscles: ["Upper Back", "Biceps", "Forearms", "Hamstrings"],
    tips: [
      "Grip the rope hand-over-hand and sit back into a low athletic stance",
      "Pull with your whole body — use legs and back, not just arms",
      "Keep a steady rhythm and don't let the rope go slack between pulls"
    ],
    mistakes: [
      "Standing upright and pulling only with the arms",
      "Losing grip by not maintaining a firm hand-over-hand technique"
    ]
  },
  "Burpee Broad Jump": {
    muscles: ["Full Body", "Quadriceps", "Glutes", "Chest", "Core"],
    tips: [
      "Drop into the burpee fast, chest touches the floor",
      "Explode up and immediately transition into a forward broad jump",
      "Land softly with bent knees and reset quickly for the next rep"
    ],
    mistakes: [
      "Not fully extending the hips on the broad jump, losing distance",
      "Taking too long between the burpee and the jump — flow should be continuous"
    ]
  },
  "Rowing Machine": {
    muscles: ["Legs", "Back", "Biceps", "Core"],
    tips: [
      "Drive with your legs first, then lean back, then pull with arms — legs-body-arms sequence",
      "Keep a firm grip and pull the handle to your lower ribs",
      "On the return, reverse the sequence: arms-body-legs to slide forward smoothly"
    ],
    mistakes: [
      "Pulling with the arms before the legs have finished driving",
      "Rounding the back instead of maintaining a proud chest throughout"
    ]
  },
  "Farmers Carry": {
    muscles: ["Forearms", "Traps", "Core", "Glutes"],
    tips: [
      "Stand tall with shoulders back and down — don't shrug up",
      "Take short, controlled steps and keep your core braced tight",
      "Grip the handles as hard as you can and breathe rhythmically"
    ],
    mistakes: [
      "Leaning to one side or letting the weights pull you forward",
      "Taking strides that are too long which causes wobbling"
    ]
  },
  "Sandbag Lunge": {
    muscles: ["Quadriceps", "Glutes", "Hamstrings", "Core"],
    tips: [
      "Hold the sandbag in a bear hug position tight against your chest",
      "Take a moderate stride and lower until the back knee nearly touches the floor",
      "Drive through the front heel and keep your torso as upright as possible"
    ],
    mistakes: [
      "Letting the sandbag slip down which pulls you forward",
      "Rushing and losing balance — steady controlled steps are faster overall"
    ]
  },
  "Wall Ball": {
    muscles: ["Quadriceps", "Glutes", "Shoulders", "Core"],
    tips: [
      "Squat deep with the ball at your chest, then drive up explosively",
      "Use the hip drive to launch the ball — arms just guide it to the target",
      "Catch the ball and immediately flow into the next squat rep"
    ],
    mistakes: [
      "Not squatting deep enough which reduces power on the throw",
      "Pressing the ball with your arms instead of using leg drive"
    ]
  },
  "Running": {
    muscles: ["Quadriceps", "Hamstrings", "Calves", "Core"],
    tips: [
      "Maintain an upright posture with a slight forward lean from the ankles",
      "Land with a midfoot strike under your center of mass",
      "Keep a steady, sustainable pace — consistent splits are key in Hyrox"
    ],
    mistakes: [
      "Starting too fast and burning out before the later rounds",
      "Over-striding which wastes energy and increases injury risk"
    ]
  },
  "Tempo Run": {
    muscles: ["Quadriceps", "Hamstrings", "Calves", "Cardiovascular System"],
    tips: [
      "Run at a 'comfortably hard' pace — you can speak in short phrases but not full sentences",
      "Focus on maintaining an even pace throughout the entire duration",
      "Use breathing rhythm to gauge effort — try a 2:2 or 3:3 inhale:exhale pattern"
    ],
    mistakes: [
      "Starting too fast and fading in the second half",
      "Running too easy — tempo should feel like sustained effort, not a jog"
    ]
  },
  "Interval Run": {
    muscles: ["Quadriceps", "Hamstrings", "Calves", "Cardiovascular System"],
    tips: [
      "Hit your target pace during work intervals — don't hold back",
      "Use rest intervals for active recovery (walk or light jog), not complete stops",
      "Focus on maintaining consistent speed across all work intervals"
    ],
    mistakes: [
      "Going all-out on the first interval and crashing on later ones",
      "Resting too long between intervals and losing the training stimulus"
    ]
  },
  "Push-Up": {
    muscles: ["Chest", "Triceps", "Front Delts", "Core"],
    tips: [
      "Keep your body in a straight line from head to heels — squeeze your glutes",
      "Lower until your chest touches or nearly touches the floor",
      "Keep your elbows at about 45 degrees, not flared out to 90"
    ],
    mistakes: [
      "Sagging the hips and letting the lower back hyperextend",
      "Only doing half reps and not touching the chest to the floor"
    ]
  },
  "Goblet Squat": {
    muscles: ["Quadriceps", "Glutes", "Core"],
    tips: [
      "Hold the dumbbell or kettlebell close to your chest with elbows pointing down",
      "Squat deep — elbows should track inside the knees at the bottom",
      "Drive up through the full foot and squeeze your glutes at the top"
    ],
    mistakes: [
      "Letting the weight pull you forward and rounding the upper back",
      "Not squatting deep enough to get full quad and glute engagement"
    ]
  },
  "Barbell Row": {
    muscles: ["Upper Back", "Lats", "Biceps", "Rear Delts"],
    tips: [
      "Hinge forward at about 45 degrees and pull the bar to your lower chest",
      "Squeeze your shoulder blades together at the top of each rep",
      "Keep your core braced and avoid jerking the torso upright"
    ],
    mistakes: [
      "Using too much body English and heaving the weight with momentum",
      "Rounding the lower back by hinging too far forward"
    ]
  }
};
